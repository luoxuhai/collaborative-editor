...
class ImageHandlers {
  ...
  imageDropAndPasteHandler(imageDataUrl, type) {
    this.handleContentChangeForImage().then((imageIndex) => {
      ...
      this.editor.options.image.handlers
        .imageDataURIUpload(imageDataUrl, type)
        .then((imageUrl) => {
          self.replaceImagePlaceholderWithImage(
            placeholderId,
            deltaId,
            imageUrl
          );
  ...
  imageUploadButtonHandler() {
    ...
    fileInput.onchange = () => {
      ...
      this.handleContentChangeForImage().then((imageIndex) => {
        ...
        self
          .readFileAsDataURI(files[0])
          .then((dataURI) => {
            self.previewInImagePlaceholder(placeholderId, dataURI);

            self.editor.options.image.handlers
              .imageDataURIUpload(files[0])
              .then((imageUrl) => {
                self.replaceImagePlaceholderWithImage(
                  placeholderId,
                  deltaId,
  ...
  insertImagePlaceholder(id, index) {
    ...
    return this.editor.composition.addLocalOnlyDelta(
      placeholderDelta,
      placeholderRevertDelta
    );
  }
