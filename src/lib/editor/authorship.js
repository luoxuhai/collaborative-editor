...
class Authorship {
  constructor(editor, composition, options) {
    ...
    this.authorSidebar = new AuthorSidebar(
      editor.quill,
      AuthorAttribute,
      options
    );
...
class AuthorSidebar {
  ...
  update(delta, oldDelta) {
    ...
    for (let i = 0; i < delta.ops.length; i++) {
      ...
      if (op.insert) {
        ...
        let afterDelete = parallelDelta.compose(
          new Delta().retain(index).delete(op.delete)
        );
    ...
    if (current) {
      ...
      if (!this.authorsInfo[lineAuthorId]) {
        ...
        self.options.handlers
          .getAuthorInfoById(lineAuthorId)
          .then((author) => {
            self.authorsInfo[lineAuthorId] = author;
            self.updateAuthorInfoOnSidebarItem(
              sidebarItem,
              lineIndex,
              lineAuthorId
            );
        ...
        this.updateAuthorInfoOnSidebarItem(
          sidebarItem,
          lineIndex,
          lineAuthorId
        );
  ...
  updateAuthorInfoOnSidebarItem(sidebarItem, itemIndex, authorId) {
    ...
    if (authorId) {
      let authorName = this.authorsInfo[authorId].nickname;
  ...
  adjustSidebarItemPosition(line, lineIndex) {
    ...
    this.sidebarItems[lineIndex].style.marginTop = styles.getPropertyValue(
      'margin-top'
    );
    this.sidebarItems[lineIndex].style.marginBottom = styles.getPropertyValue(
      'margin-bottom'
    );
  }
  ...
  createSidebarItem(afterIndex) {
    ...
    item.innerHTML =
      '<div class="color-bar"></div><div class="author-name"></div>';
  ...
  deleteSidebarItem(afterIndex, count) {
    ...
    if (
      previousItem.getAttribute('data-author-id') !==
      nextItem.getAttribute('data-author-id')
    ) {
    ...
    delta.ops.forEach((op) => {
      if (op.attributes && op.attributes.author) {
        ...
        if (
          ...
          typeof self.identifiedAuthorIds[authorId] === 'undefined'
        ) {
          // New author identified.
          // Assign a color to this author.
          self.identifiedAuthorIds[authorId] =
            self.identifiedAuthorCount++ % self.colors.length;
  ...
  addStyleForAuthor(authorId, preferredColor) {
    ...
    if (color) {
      let rule =
        `.ql-author-sidebar .ql-author-${authorId} .color-bar { ` +
        `background-color:${color}; }\n`;
      this.styleElement.sheet.insertRule(rule, 0);
      let rule2 =
        `.ql-author-sidebar .ql-author-${authorId} .author-name { ` +
        `color:${color}; }\n`;
  ...
  findColor(authorId) {
    if (typeof this.identifiedAuthorIds[authorId] === 'undefined') return null;
