...
class Composition {
  ...
  flush(oldDelta) {
    ...
    let [localChangeDelta, finalSubmittedDelta] = this.handleSubmitDeltaMerge(
      upstreamDelta
    );
    let changeDelta = upstreamDelta.compose(finalSubmittedDelta);

    oldDelta = this.handleLocalDeltaMerge(
      upstreamDelta,
      localChangeDelta,
      finalSubmittedDelta,
      oldDelta
    ...
    if (upstreamDelta.ops.length !== 0) {
      this.editor.dispatchEvent(EditorEvents.upstreamTextChanged, {
        delta: upstreamDelta,
        oldDelta: oldDelta,
      });
    }

    this.editor.dispatchEvent(EditorEvents.userTextChanged, {
      delta: finalSubmittedDelta,
      oldDelta: oldDelta.compose(upstreamDelta),
    });
    this.editor.dispatchEvent(EditorEvents.editorTextChanged, {
      delta: changeDelta,
      oldDelta: oldDelta,
    });
  }
  ...
  handleLocalDeltaMerge(
    ...
    if (upstreamDelta && upstreamDelta.ops.length !== 0) {
      ...
      let localFixingDelta = revertDelta
        .compose(upstreamDelta)
        .compose(finalSubmittedDelta);
  ...
  handleSubmitDeltaMerge(upstreamDelta) {
    ...
    let transformedPendingSubmitDelta = upstreamDelta.transform(
      pendingSubmitDelta,
      true
    );
    ...
    this.editor.dispatchEvent(
      EditorEvents.beforeSubmitToUpstream,
      transformedPendingSubmitDelta
    );
  ...
  addLocalOnlyDelta(
    changeDelta,
    revertDelta,
    convertAgainstLocalOnlyDelta = true
  ) {
    let convertedChange = convertAgainstLocalOnlyDelta
      ? this.localOnlyDelta.change.transform(changeDelta, true)
      : changeDelta;
    let convertedRevert = convertAgainstLocalOnlyDelta
      ? this.localOnlyDelta.change.transform(revertDelta, true)
      : revertDelta;
  ...
  removeLocalOnlyDelta(id) {
    let idx = this.localOnlyDelta.steps.findIndex((element) => {
  ...
  updateLocalOnlyDelta() {
    ...
    for (let i = 0, l = this.localOnlyDelta.steps.length; i < l; i++) {
      this.localOnlyDelta.change = this.localOnlyDelta.change.compose(
        this.localOnlyDelta.steps[i].change
      );
    }

    for (let i = this.localOnlyDelta.steps.length - 1; i >= 0; i--) {
      this.localOnlyDelta.revert = this.localOnlyDelta.revert.compose(
        this.localOnlyDelta.steps[i].revert
      );
    }
  }
